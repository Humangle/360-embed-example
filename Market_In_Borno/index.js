...
let main = async (view) => {
	...
	const canvas = document.getElementById("c_Market_In_Borno");
	...
	//loading textures
	const loadingElem = document.querySelector('#loading_Market_In_Borno');
			...
			const onTouchEnd = (event) => {
				...
				for ( let i = 0; i < intersections.length; i++ ) {
					...
					if (intersections[i].object.name != "sphere"){
						document.getElementById("c_Market_In_Borno").style.cursor = "pointer";
						this.selectedObject = intersections[i].object;
						intersections[i].object.material.opacity = 1;
						teleport(intersections[i].object.name);
					}
				}
				if ((intersections.length == 0) && (document.getElementById("c_Market_In_Borno").style.cursor == "pointer")){
					document.getElementById("c_Market_In_Borno").style.cursor = "grab";
				}
			}
	...
	class MousePickHelper extends THREE.EventDispatcher {
		constructor(scene) {
			...
			const onPointerDown = (event) => {
				...
				document.getElementById("c_Market_In_Borno").style.cursor = "grabbing";
			}
...
document.getElementById('launchFS_Market_In_Borno').addEventListener('click', (event) => {
    var cel = document.getElementById('c_Market_In_Borno');
...
document.getElementById('c_Market_In_Borno').addEventListener('fullscreenchange', () => {
	...
	if ('xr' in navigator) {
		navigator.xr.isSessionSupported('immersive-ar').then(function(supported) {
			if (supported) {
				...
				document.getElementById('launchVR_Market_In_Borno').style.display = "block";
				async function onSessionStarted( session ) {
					session.addEventListener( 'end', onSessionEnded );
					renderer.xr.setReferenceSpaceType( 'local' );
					await renderer.xr.setSession( session );
					document.getElementById('launchVR_Market_In_Borno').style.display = "none";
				...
				document.getElementById('launchVR_Market_In_Borno').addEventListener('click', (event) => {
...
let links;
fetch('./Market In Borno.hvrj').then(response => response.json()).then(hvrj => {
	links = hvrj;
