...
let main = async (view) => {
	...
	//loading textures
	const loadingElem = document.querySelector('#loading_Outside_The_Camp_2');
	const progressBarElem = loadingElem.querySelector('.progressbar');
	const loadManager = new THREE.LoadingManager();
	const loader = new THREE.ImageBitmapLoader(loadManager);
	loader.setOptions( { imageOrientation: 'flipY' } );
	
	const sT = await loader.loadAsync("https://raw.githubusercontent.com/Humangle/VRTourEditor/refs/heads/main/assets/no-image.jpg");
	...
	class MousePickHelper extends THREE.EventDispatcher {
		constructor(scene) {
			...
			const onPointerUp = (event) => {
				...
				document.getElementById("c_Outside_The_Camp_2").style.cursor = "grab";
			}
			...
			const onPointerDown = (event) => {
				...
				document.getElementById("c_Outside_The_Camp_2").style.cursor = "grabbing";
			}
		...
		update(pickablesParent, time){
			...
			for ( let i = 0; i < intersections.length; i++ ) {
				if (intersections[i].object.name != "sphere"){
					document.getElementById("c_Outside_The_Camp_2").style.cursor = "pointer";
					this.selectedObject = intersections[i].object;
					intersections[i].object.material.opacity = 1;
				}
			}
			if ((intersections.length == 0) && (document.getElementById("c_Outside_The_Camp_2").style.cursor == "pointer")){
				document.getElementById("c_Outside_The_Camp_2").style.cursor = "grab";
			}
		}
	}
...
document.getElementById('launchFS_Outside_The_Camp_2').addEventListener('click', (event) => {
    var cel = document.getElementById('c_Outside_The_Camp_2');
...
document.getElementById('c_Outside_The_Camp_2').addEventListener('fullscreenchange', () => {
	...
	if ('xr' in navigator) {
		navigator.xr.isSessionSupported('immersive-ar').then(function(supported) {
			if (supported) {
				...
				document.getElementById('launchVR_Outside_The_Camp_2').style.display = "block";
				async function onSessionStarted( session ) {
					session.addEventListener( 'end', onSessionEnded );
					renderer.xr.setReferenceSpaceType( 'local' );
					await renderer.xr.setSession( session );
					document.getElementById('launchVR_Outside_The_Camp_2').style.display = "none";
				...
				document.getElementById('launchVR_Outside_The_Camp_2').addEventListener('click', (event) => {
...
let links;
fetch('./Outside The Camp 2.hvrj').then(response => response.json()).then(hvrj => {
	links = hvrj;
