...
let main = async (view) => {
	...
	const canvas = document.getElementById("c_Outside_The_Camp_1");
	...
	//loading textures
	const loadingElem = document.querySelector('#loading_Outside_The_Camp_1');
	...
	class MousePickHelper extends THREE.EventDispatcher {
		constructor(scene) {
			...
			const onPointerUp = (event) => {
				...
				document.getElementById("c_Outside_The_Camp_1").style.cursor = "grab";
			}
			...
			const onPointerDown = (event) => {
				...
				document.getElementById("c_Outside_The_Camp_1").style.cursor = "grabbing";
			}
		...
		update(pickablesParent, time){
			...
			for ( let i = 0; i < intersections.length; i++ ) {
				if (intersections[i].object.name != "sphere"){
					document.getElementById("c_Outside_The_Camp_1").style.cursor = "pointer";
					this.selectedObject = intersections[i].object;
					intersections[i].object.material.opacity = 1;
				}
			}
			if ((intersections.length == 0) && (document.getElementById("c_Outside_The_Camp_1").style.cursor == "pointer")){
				document.getElementById("c_Outside_The_Camp_1").style.cursor = "grab";
			}
		}
	}
...
document.getElementById('launchFS_Outside_The_Camp_1').addEventListener('click', (event) => {
    var cel = document.getElementById('c_Outside_The_Camp_1');
...
document.getElementById('c_Outside_The_Camp_1').addEventListener('fullscreenchange', () => {
	...
	if ('xr' in navigator) {
		navigator.xr.isSessionSupported('immersive-ar').then(function(supported) {
			if (supported) {
				...
				document.getElementById('launchVR_Outside_The_Camp_1').style.display = "block";
				async function onSessionStarted( session ) {
					session.addEventListener( 'end', onSessionEnded );
					renderer.xr.setReferenceSpaceType( 'local' );
					await renderer.xr.setSession( session );
					document.getElementById('launchVR_Outside_The_Camp_1').style.display = "none";
				...
				document.getElementById('launchVR_Outside_The_Camp_1').addEventListener('click', (event) => {
...
let links;
fetch('./Outside The Camp 1.hvrj').then(response => response.json()).then(hvrj => {
	links = hvrj;
