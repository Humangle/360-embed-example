...
let main = async (view) => {
	...
	//loading textures
	const loadingElem = document.querySelector('#loading_Demolished_Camp');
	...
	class MousePickHelper extends THREE.EventDispatcher {
		constructor(scene) {
			...
			const onPointerDown = (event) => {
				...
				document.getElementById("c_Demolished_Camp").style.cursor = "grabbing";
			}
...
document.getElementById('launchFS_Demolished_Camp').addEventListener('click', (event) => {
    var cel = document.getElementById('c_Demolished_Camp');
...
document.getElementById('c_Demolished_Camp').addEventListener('fullscreenchange', () => {
	...
	if ('xr' in navigator) {
		navigator.xr.isSessionSupported('immersive-ar').then(function(supported) {
			if (supported) {
				...
				document.getElementById('launchVR_Demolished_Camp').style.display = "block";
				async function onSessionStarted( session ) {
					session.addEventListener( 'end', onSessionEnded );
					renderer.xr.setReferenceSpaceType( 'local' );
					await renderer.xr.setSession( session );
					document.getElementById('launchVR_Demolished_Camp').style.display = "none";
				...
				document.getElementById('launchVR_Demolished_Camp').addEventListener('click', (event) => {
...
let links;
fetch('./Demolished Camp.hvrj').then(response => response.json()).then(hvrj => {
	links = hvrj;
